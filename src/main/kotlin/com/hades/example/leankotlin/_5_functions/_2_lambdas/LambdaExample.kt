...
private fun example3() {
    ...
    // E.g., 函数类型作为形参 - 不带参数
    fun plus(n: Int, callback: () -> Unit): Int {
        callback.invoke()
        return (n + 1)
    }
    // 如何调用？
    ...
    val num = 2;
    val printFunc: () -> Unit = {
        println("plus invoked")
    }
    val result = plus(num, callback = printFunc)
    println("after plus, result=$result")
    // OR
    val result2 = plus(num, callback = {
        println("plus invoked")
    })
    println("after plus, result=$result2")
    // OR
    val result3 = plus(num, {
        println("plus invoked")
    })
    println("after plus, result=$result3")

    // 方式2:用函数体外部来传递函数类型实参
    val result4 = plus(num) {
        println("plus invoked")
    }
    println("after plus, result=$result4")

    // E.g., 函数类型作为形参 - 带参数和返回值
    fun total(n1: Int, n2: Int, n3: Int, callback: (Int, Int) -> Int): Int {
        return n1 + callback.invoke(n2, n3)
    }

    val sum: (Int, Int) -> Int = { /* 此处Int可以省略 */ n1: Int, n2: Int ->
        n1 + n2
    }
    // 如何调用？
    // 方式1:用参数来传递函数类型实参
    val total = total(2, 3, 4, callback = sum)
    println("total=$total") // log: total=9
    // OR
    val total2 = total(2, 3, 4, callback = {/* 此处Int可以省略 */ num1: Int, num2: Int ->
        num1 + num2
    }) // Recommended
    println("total=$total2") // log: total=9
    val total3 = total(2, 3, 4, {/* 此处Int可以省略 */ num1: Int, num2: Int ->
        num1 + num2
    }) //Deprecated
